...
#define ButtonMask	(0x40)
 ...
 void WaitFunc(uint32_t);
 void timerWaitUs(uint32_t);

#define timer_trigg	(2)
...
#define GetTicks(us_wait)   ((uint32_t)(((us_wait*0.000001*CMU_ClockFreqGet(cmuClock_LFA))/4)))
#define CompValue 	((uint32_t)(CMU_ClockFreqGet(cmuClock_LFA)/(4*TimeSet)))
#define log_val()				(float)(roll+(0.001*((LETIMER_CounterGet(LETIMER0)*4*1000)/CMU_ClockFreqGet(cmuClock_LFA))))

 ...
 */
uint32_t loggerGetTimestamp(uint8_t roll)
{
	uint32_t milli = 1000*((3*roll)+((((((CMU_ClockFreqGet(cmuClock_LFA)/(divide*1))*(TOTAL_P)))-LETIMER_CounterGet(LETIMER0))*divide)/32768));//return timerGetRunTimeMilliseconds();
	return milli;
}
